...
bool vote(int voter, int rank, string name)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            // If candidate found, update preference using matrix (colum = rank / line = voter)
...
void tabulate(void)
{
    ...
    // Update vote counts for highest preference for all non-eliminated candidates
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
...
int find_min(void)
{
    // Return the minimum number of votes of anyone still in the election to eliminate
...
bool is_tie(int min)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // If the election is tied between all remaining candidates
        if (!candidates[i].eliminated && candidates[i].votes != min)
        {
            return false;
        }
